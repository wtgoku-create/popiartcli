		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !noAgentConfig {
				for _, agent := range normalizedAgents {
					path, err := writeAgentEnvFile(agent, cfg)
					if err != nil {
...
func writeAgentEnvFile(agent string, cfg config.Config) (string, error) {
	path := filepath.Join(config.Dir(), "agents", agent, "env.sh")
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
		return "", err
	}

	var lines []string
	lines = append(lines, "# Generated by `popiart bootstrap`.")
	lines = append(lines, fmt.Sprintf("export POPIART_CONFIG_DIR=%q", config.Dir()))
	lines = append(lines, fmt.Sprintf("export POPIART_ENDPOINT=%q", cfg.Endpoint))
	if cfg.Project != "" {
		lines = append(lines, fmt.Sprintf("export POPIART_PROJECT=%q", cfg.Project))
	}
	lines = append(lines, "")
...
func bootstrapNextSteps(result bootstrapResult) []string {
	...
	for _, file := range result.GeneratedFiles {
		if file.Kind == "completion" {
			switch file.Name {
			case "zsh":
				steps = append(steps, fmt.Sprintf("执行 `source %s` 立即启用 zsh completion", file.Path))
			case "bash":
				steps = append(steps, fmt.Sprintf("执行 `source %s` 立即启用 bash completion", file.Path))
			case "fish":
				steps = append(steps, fmt.Sprintf("执行 `source %s` 立即启用 fish completion", file.Path))
			case "powershell":
				steps = append(steps, fmt.Sprintf("在 PowerShell 中执行 `. %s` 立即启用 completion", file.Path))
			}
		}
