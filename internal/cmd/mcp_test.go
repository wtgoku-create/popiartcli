...
func TestMCPServerInitializeAndToolsList(t *testing.T) {
	...
	if !foundUpload {
		t.Fatalf("expected tools/list to include upload_artifact, got %#v", tools)
	}
}
...
func TestMCPServerUploadArtifactToolCall(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, `{"ok":true,"data":{"id":"art_mcp_upload_1","filename":"agent-chat.png","content_type":"image/png","size_bytes":8,"created_at":"2026-03-28T04:05:00Z","expires_at":"2026-04-27T04:05:00Z"}}`)
	}))
	defer server.Close()
